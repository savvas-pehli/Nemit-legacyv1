...
FROM new_test.prefecture_fuel_con;"""

GET_FUEL_COLUMNS_QUERY = """
SELECT column_name
FROM information_schema.columns
WHERE table_schema = 'new_test'
...
SELECT {columns}
FROM clean FORCE INDEX (idx_stat_year_month_day)
WHERE Station IN {stations}
  AND {year_condition}
...
SELECT year,Hour,municipality,`{air_pollutant}` FROM new_test.aggr_choro_per_hour_year
where region={region};
"""
CHOROPLETH_YEARLY_QUERY = """
SELECT year,municipality,`{air_pollutant}` FROM new_test.aggr_choro_per_year
where region={region};
"""
...
SUB_ECON_QUERY = """SELECT DISTINCT(`economic activity`) FROM gas_econ_activity
                        WHERE `code name` REGEXP '^[{code_name_join}]' 
                        AND CHAR_LENGTH(`code name`) > 1;"""
