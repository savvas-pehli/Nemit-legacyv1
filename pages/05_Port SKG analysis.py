...
from services.port_services import fetch_aggregated_port_data, get_port_metadata
from utils.constants import DAYS_LIST, DAYS_MAP, MONTH_MAP, MONTHS_LIST, VALID_TABLES
...
selected_metrics = st.multiselect(
    "Select Metrics to Analyze:", options=meta["metrics"], max_selections=2
)
...
if min_y == max_y:
    ...
    year_range = st.sidebar.slider(
        "Select Year Range:", min_value=min_y, max_value=max_y, value=(min_y, max_y)
    )

ui_month_range = st.sidebar.select_slider(
    "Select Month Range", options=MONTHS_LIST, value=("January", "December")
)
db_month_range = (MONTH_MAP[ui_month_range[0]], MONTH_MAP[ui_month_range[1]])

ui_day_range = st.sidebar.select_slider(
    "Select Day of Week", options=DAYS_LIST, value=("Monday", "Sunday")
)
    ...
    with st.spinner("Extracting insights from warehouse..."):
        try:
            ...
            if timeframe == "Day":
                ...
                df_result[timeframe] = pd.Categorical(
                    df_result[timeframe], categories=DAYS_LIST, ordered=True
                )
