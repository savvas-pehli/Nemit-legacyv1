...
selected_timeframe = st.sidebar.selectbox(
    "Please choose timeframe", list(timeframe_options.keys())
)
...
raw_gdf = fetch_choropleth_data(
    conn, region, selected_col, selected_timeframe, valid_pollutants
)
    ...
    with st.spinner("Processing spatial polygons and aggregating metrics..."):
        try:
            ...
            if selected_timeframe == "Yearly timeframe":
                ...
                processed_df = hourly_df_with_polars(
                    raw_gdf, selected_col, selected_timeframe
                )
            else:
                processed_df = hourly_df_with_polars(
                    raw_gdf, selected_col, selected_timeframe, year_input
                )
            ...
            processed_df["municipality"] = processed_df["municipality"].apply(
                greek_to_latin
            )
