...
from utils.processing import check_prefecture, check_region
...
selected_regions = st.multiselect(
    "Please select Region/s:", sorted(regions), max_selections=3
)
region_check = st.checkbox(
    "Enable Region Grouping", value=False, key="region_checked", on_change=check_region
)
...
selected_prefectures = st.multiselect(
    "Please select Prefecture/s:", sorted(prefectures), max_selections=3
)
prefecture_check = st.checkbox(
    "Enable Prefecture Grouping",
    value=False,
...
selected_fuels = st.sidebar.multiselect(
    "Select Fuel Types", fuel_columns, max_selections=2
)
...
if regional_check or prefectural_check:
    ...
    st.info(
        "In order to continue, please check either the prefecture or region box and select at least one corresponding area."
    )
